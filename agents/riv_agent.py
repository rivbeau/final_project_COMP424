...
class RivAgent(Agent):
  ...
  def step(self, chess_board, player, opponent):
    ...
    self.w_piece = 1.415
    self.w_edge = 1.357
    self.w_adjblock = 1.70
    self.w_centrality = -0.1624
    self.w_risk = 1.714
    self.t_piece = 1.3
    self.t_edge = 1.04
    self.t_adjblock = 0.887
    self.t_centrality = 0.58
    self.t_risk = 0.596
    ...
    self.w_piece *= self.t_piece
    self.w_edge *= self.t_edge
    self.w_adjblock *= self.t_adjblock
    self.w_centrality *= self.t_centrality
    self.w_risk *= self.t_risk
  ...
  def max_value(self, chess_board, alpha, beta, current_depth, max_depth):
    ...
    if is_end or current_depth >= max_depth:
      score = self.evaluate_board(chess_board, p1_score, p2_score, self.player, self.opponent) # Getting the score
  ...
  def evaluate_board(self, board,p1_score, p2_score, color, opponent):
    ...
    return self.w_piece * piece_diff  + self.w_edge *edge_control + self.w_adjblock * adj_block  + self.w_centrality * centrality_bonus  + self.w_risk * risk 