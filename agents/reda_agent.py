...
class RedaAgent(Agent):
  ...
  def max_value(self, chess_board, alpha, beta, current_depth, max_depth, start_time):
      ...
      if is_end or current_depth >= max_depth:
        return self.evaluate_board(chess_board, p1_score, p2_score)
  ...
  def evaluate_board(self, chess_board, p1_score, p2_score):
      """
      Simple evaluation function based on disc count difference.
      """
      # Get my score and opponent score based on player number
      if self.player == 1:
        my_score = p1_score
        opp_score = p2_score
      else:
      ...
      # Return difference (positive is good for us, negative is bad)
